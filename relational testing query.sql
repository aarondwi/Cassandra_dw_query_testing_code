...
WITH cte as (
	...
	WHERE dd.tahun_ajaran BETWEEN '2014/2015' AND '2016/2017'
	GROUP BY tf.kode_anggota,jd.nama_jurusan,jd.fakultas,dd.tahun_ajaran
)
SELECT kode_anggota,nama_jurusan,fakultas,tahun_ajaran,jumlah FROM cte WHERE r<=3 ORDER BY tahun_ajaran,jumlah;

--query 2 (tested)
SELECT dd.tahun,dd.bulan,SUM(tf.denda) "Total Denda",(SUM(tf.denda)-SUM(tf.terbayar)) "Belum Terbayar"
FROM transaksi_fact tf
JOIN date_dim dd ON tf.tgl_pinjam_key=dd.date_key
WHERE dd.tahun_ajaran='2015/2016'
GROUP BY dd.tahun,dd.bulan
...
JOIN jurusan_dim jd ON tf.jurusan_key=jd.jurusan_key
WHERE bd.kelompok_kategori IN ('Geologi','Tata Kota dan Pertamanan') AND dd.tahun_ajaran BETWEEN '2013/2014' AND '2015/2016' AND jd.nama_jurusan='Teknik Arsitektur' 
GROUP BY bd.judul,bd.status_sekarang,bd.nama_koleksi
ORDER BY COUNT(tf.transaksi_key) DESC
LIMIT 5;

--query 5 (tested)
SELECT dd.tahun_ajaran,dd.semester,COUNT(tf.transaksi_key),COUNT(tf.transaksi_key) - FLOOR(AVG(COUNT(tf.transaksi_key)) OVER (PARTITION BY dd.tahun_ajaran ORDER BY dd.tahun_ajaran)) AS "Selisih dengan Rata-rata tahunan"
FROM transaksi_fact tf
JOIN date_dim dd ON tf.tgl_pinjam_key=dd.date_key
JOIN jurusan_dim jd ON tf.jurusan_key=jd.jurusan_key
WHERE jd.nama_jurusan='Teknik Industri' AND dd.tahun_ajaran BETWEEN '2013/2014' AND '2015/2016'
...
AND dd.tahun_ajaran BETWEEN '2014/2015' AND '2015/2016'
AND status_sekarang='hilang'
GROUP BY bd.kelompok_kategori,bd.judul,bd.kode_judul,bd.kode_buku
ORDER BY COUNT(tf.transaksi_key) DESC;

--query 7 (tested)
SELECT uf.penerbit,COUNT(uf.*) AS "jumlah usulan untuk penerbit ini"
FROM usulan_fact uf 
